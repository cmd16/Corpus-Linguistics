...
def find_prototypical(comp_freqdist, source_dir, filenamelist, outfilename, n=5, extension=False):
    """
    ...
    :return:
    """
    # TODO: make more efficient/less redundant (don't store to file then read in file)
    file_data = []
    in_file = open(filenamelist)
    for line in in_file:
        id = line.strip()
        if extension:
        ...
        freqdist_to_wordlistfile(freqdist, "temp_wordlist.txt")
        sum = 0
        keyword_dict0, [corpus1name, types1, tokens1], [corpus2name, types2, tokens2] = keyword_tuple_from_wordlists("temp_wordlist.txt",
            comp_freqdist, p=0)
        for word in keyword_dict0:
            keyness = keyword_dict0[word][0]
            sum += keyness
        keyword_dict1 = keyword_tuple_from_wordlists(comp_freqdist, "temp_wordlist.txt", p=0)[0]
