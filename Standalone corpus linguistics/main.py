...
def freq_from_txt(infile, case_sensitive=False):
    """
    ...
    try:
        ...
        return False
    freqdist = freq_from_str(text, case_sensitive)
    return freqdist
...
def dir_to_wordlists(src, dest, case_sensitive=False):
    for filename in os.listdir(src):
        if filename.endswith(".txt"):
            print(filename)  # TODO: take out print?
            freqdist = freq_from_txt(os.path.join(src, filename), case_sensitive=case_sensitive)
            new_name = os.path.join(dest, filename.replace(".txt","_wordlist.txt"))
            freqdist_to_wordlistfile(freqdist, new_name)
...
def keyword_tuple_from_freqdists(freqdist1, freqdist2, p=0.01, corpus1name="corpus 0", corpus2name="corpus 1"):
    """
...
def find_prototypical(comp_freqdist, source_dir, idlist, outfilename, n=10, extension="", case_sensitive=False, use_wordlist=False):
    """
    Find the n most prototypical files as compared to a FreqDist by performing keyword analysis and store those filenames
    :param comp_freqdist: the FreqDist to compare
    :param source_dir: the directory to read files from
    :param idlist: a file with ids (relative filenames) to read. Must be filename.
    ...
    for line in f_in:
        id = line.strip()
        if extension:
            filename = os.path.join(source_dir, "%s%s" % (id, extension))
        else:
            filename = os.path.join(source_dir, id)
        print(id, "freqdist")
        if use_wordlist:
            freqdist = wordlist_to_freqdist(filename)
        else:
            freqdist = freq_from_txt(filename, case_sensitive=case_sensitive)
        if not freqdist:
            continue
        sum = 0
        print(id, "keywords")
    ...
    end_num = min(n, len(file_data))
    for i in range(end_num):
        out_file.write("%s\t%f\n" % (file_data[i][0], file_data[i][1]))
    out_file.write("\n")
    for i in range(-1, end_num*-1 - 1, -1):
        out_file.write("%s\t%f\n" % (file_data[i][0], file_data[i][1]))
    out_file.close()
    return file_data
...
if __name__ == "__main__":
    ...
    fanfic_wordlist_dir = os.path.join(proj_dir, "Fanfic_wordlists")
    fanfic_list_dir = os.path.join(proj_dir, "Fanfic lists")
    wordlist_dir = os.path.join(proj_dir, "wordlists")
    prototypical_dir = os.path.join(proj_dir, "Prototypical")

    # dir_to_wordlists(os.path.join(proj_dir, "Fanfic_all"), fanfic_wordlist_dir)

    for idlist in os.listdir(fanfic_list_dir):
        if idlist.endswith(".txt"):
            print(idlist)
            freqdist = wordlist_to_freqdist(os.path.join(wordlist_dir, idlist.replace(" ", "_").replace(".txt", "_python.txt")))
            find_prototypical(comp_freqdist=freqdist, source_dir=fanfic_wordlist_dir, idlist=os.path.join(fanfic_list_dir, idlist),
                              outfilename=os.path.join(prototypical_dir, idlist), n=1000000, extension="_wordlist.txt", use_wordlist=True)
