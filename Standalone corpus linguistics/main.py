...
def star_trek_tag():
    tag_lemma = tag_lemma_from_tree_directory(os.path.join(project_dir, "Tag/Tag Fanfic/Tagged Star Trek"))
    store_taglemma_results(tag_lemma, filename=os.path.join(project_dir,
                                                            "Antconc results/Tag results/Tag Fanfic/Star Trek POS Lemma.xlsx"))
...
def original_canon_freq():
    ...
    for dir_name in os.listdir(current)[1:]:
        print(dir_name)
        print(os.path.join(project_dir, dir_name + " wordlist"))
        freqdist = freq_from_dir(os.path.join(current, dir_name), walk=True)
        freqdist_to_wordlistfile(freqdist, os.path.join(project_dir, dir_name + "Original Canon wordlist"))
...
def dw_wordlists_keyword():
    ...
    keyword_dict = keyword_analysis_from_wordlists(os.path.join(resultdir, "Doctor Who Related wordlist.txt"), os.path.join(resultdir, "DW_related_antconc_results.txt"))
...
def fanfic_wordlists():
    fanficdir = os.path.join(project_dir, "Fanfiction")
    for item in os.walk(fanficdir):
        path = item[0]
        # fandompath = path[:path.index("works") -1]
        if path.endswith("works"):
            print("fanfic_wordlists processing", path)
            freqdist = freq_from_dir(path)
            freqdist_to_wordlistfile(freqdist, os.path.join(path, " wordlist.txt"))

def doctor_who_wordlist():
    path = os.path.join(project_dir, "Fanfiction/Doctor Who/Doctor Who works")
    freqdist = freq_from_dir(path)
    freqdist_to_wordlistfile(freqdist, os.path.join(path, "Doctor Who wordlist.txt"))

def hamilton_wordlist():
    path = os.path.join(project_dir, "Fanfiction/Hamilton/Hamilton works")
    freqdist = freq_from_dir(path)
    freqdist_to_wordlistfile(freqdist, os.path.join(path, "Hamilton wordlist.txt"))
...
def keyword_analysis_both():
    results_dir = os.path.join(project_dir, "Antconc results/AntConc")
    for fanfic_wordlist in os.listdir(os.path.join(results_dir, "Ant Fanfic/Fanfic wordlists")):
